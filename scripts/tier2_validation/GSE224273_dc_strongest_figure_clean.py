...
# Keep full-sample analysis rows only
stats = stats[stats["analysis"] == "full_samples"].copy()

# Sort for plotting: asymptomatic first, then symptomatic
...
for ax, (col, ylabel, letter) in zip(axes, panels):
    ...
    row = stats[stats["endpoint"] == col].iloc[0]
    ax.text(
        0.03, 0.97,
...
png = figdir / "Figure_GSE224273_DC_strongest_signals_clean.png"
pdf = figdir / "Figure_GSE224273_DC_strongest_signals_clean.pdf"
