...
summary_row.to_csv(results_dir / "cd8_primary_hscrp_gzmk_vd2_summary.csv", index=False)
df.to_csv(results_dir / "cd8_primary_hscrp_gzmk_vd2_input.csv", index=False)
...
pval = summary_row["p_value"].iloc[0]
beta = summary_row["beta"].iloc[0]
ax.set_xlabel("hs-CRP")
ax.set_ylabel("GZMK+ Vd2 gdT proportion")
ax.set_title(f"GZMK+ Vd2 gdT vs hs-CRP\nβ={beta:.5f}, p={pval:.4f}")
fig.tight_layout()
fig.savefig(figures_dir / "Figure_cd8_primary_hscrp_gzmk_vd2_scatter.png", dpi=300, bbox_inches="tight")
fig.savefig(figures_dir / "Figure_cd8_primary_hscrp_gzmk_vd2_scatter.pdf", bbox_inches="tight")
plt.close(fig)
...
fig.tight_layout()
fig.savefig(figures_dir / "Figure_cd8_primary_hscrp_gzmk_vd2_tertiles.png", dpi=300, bbox_inches="tight")
fig.savefig(figures_dir / "Figure_cd8_primary_hscrp_gzmk_vd2_tertiles.pdf", bbox_inches="tight")
plt.close(fig)
cat > /pub/subhajr/projects/heat_aging_human_reanalysis/scripts/cd8_primary_hscrp_gzmk_vd2gdt.py <<'PY'
from pathlib import Path
import numpy as np
import pandas as pd
import statsmodels.formula.api as smf
...
df = df.dropna(subset=["proportion", "infl.hs_crp", "subject.ageGroup", "subject.biologicalSex", "subject.bmi"]).copy()
df["log_hs_crp"] = np.log1p(df["infl.hs_crp"])
