...
features = ["gzmk_like_score", "NKG7", "GNLY", "KLRD1", "TYROBP"]
...
fig, axes = plt.subplots(nrows, ncols, figsize=(10.2, 4.2 * nrows))
...
for ax, feature, letter in zip(axes, features, letters):
    ...
    for i, (vals, grp) in enumerate(zip([x1, x2], group_order), start=1):
        ...
        ax.scatter(x, vals, s=48, color=fills[grp], edgecolors="black", linewidths=0.5, zorder=3)

    st = stats.loc[stats["feature"] == feature].iloc[0]
    ax.text(
        0.03, 0.97,
        f"P = {st['mannwhitney_p']:.3g}\nΔ = {st['No_IPH_minus_IPH']:.3f}",
...
fig.suptitle("Independent plaque validation in GSE163154", fontsize=14, fontweight="bold", y=0.99)
fig.tight_layout()

png = figdir / "Panel_GSE163154_second_plaque_validation.png"
pdf = figdir / "Panel_GSE163154_second_plaque_validation.pdf"
